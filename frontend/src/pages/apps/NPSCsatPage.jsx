...
const NPSGauge = ({ score = 62, detractors = 12, passives = 14, promoters = 74 }) => {
  ...
  return (
    <svg viewBox="0 0 240 125" className="w-full max-w-[260px] mx-auto">
      ...
      <circle cx={needle.x} cy={needle.y} r="8" fill="white" stroke="#0D0D0D" strokeWidth="2" />
      <text x={cx} y={cy - 4} textAnchor="middle" fill="white" fontSize="30" fontWeight="bold" fontFamily="Outfit">
        {score > 0 ? `+${score}` : score}
      </text>
...
const ChartTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-[#1E1E1E] border border-[#333] rounded-lg px-3 py-2 text-xs shadow-xl">
      <p className="text-[#9CA3AF] mb-1 font-medium">{label}</p>
      {payload.map(p => (
...
export default function NPSCsatPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#0C0C0C", color: "#FFFFFF", fontFamily: "'IBM Plex Sans', sans-serif" }}>

      {/* ── TOP NAV ──────────────────────────────────────────────────────── */}
      <nav style={{ background: "#111111", borderBottom: "1px solid #222222" }}>
        <div className="max-w-screen-2xl mx-auto px-6 py-0 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 py-3">
            ...
            <div>
              <div className="text-white text-[11px] font-bold tracking-[0.15em]">HITACHI SYSTEMS INDIA</div>
          ...
          <div className="flex items-center gap-1">
            {NAV_TABS.map(tab => (
              <button key={tab} data-testid={`nav-tab-${tab.toLowerCase().replace(/\s/g,'-')}`}
                ...
                style={{ color: activeTab === tab ? "#FFFFFF" : "#4B5563" }}>
                {tab}
                {activeTab === tab && (
          ...
          <div className="flex items-center gap-3">
            <button data-testid="back-to-portal-btn" onClick={() => navigate("/")}
              className="flex items-center gap-1.5 text-[#4B5563] hover:text-white text-xs transition-colors py-3">
              <ArrowLeft size={13} /><span>Portal</span>
            </button>
            <div className="flex items-center gap-1.5 text-[11px] font-bold tracking-wider" style={{ color: "#22C55E" }}>
              <div className="w-1.5 h-1.5 rounded-full bg-[#22C55E] animate-pulse" />
              LIVE
            </div>
            <button data-testid="send-survey-btn"
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold tracking-wider text-white transition-colors"
              style={{ background: "#CC0000" }}>
              <Send size={12} />SEND SURVEY
            </button>
            <button data-testid="logout-nps-btn" onClick={doLogout}
              className="text-[#4B5563] hover:text-white py-3 transition-colors">
              <LogOut size={15} />
            </button>
          </div>
        </div>
        ...
        <main className="max-w-screen-2xl mx-auto px-6 py-6 space-y-5">
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className="text-[10px] font-bold tracking-[0.2em] px-3 py-1 rounded-full border" style={{ color: "#A78BFA", borderColor: "#3D2E6B", background: "#1E1533" }}>
                  Q2 2026 · VOICE OF CUSTOMER INTELLIGENCE
                </span>
              </div>
              <h1 className="text-4xl font-bold text-white leading-tight font-['Outfit']">
                Customer Experience<br />
                <span style={{ color: "#CC0000" }}>Command Centre</span>
              </h1>
              <p className="text-[#4B5563] text-sm mt-2 max-w-xl">
              ...
              <div className="flex items-center gap-5 mt-3">
                {[["142", "Responses This Month"], ["38", "Active Accounts"], ["0", "Campaigns Running"], ["4 min ago", "Last sync"]].map(([v, l]) => (
                  <div key={l} className="flex items-center gap-1.5">
                    ...
                    <span className="text-[#4B5563] text-xs">{l}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="flex items-center gap-3 flex-shrink-0">
              <button data-testid="new-campaign-btn"
                className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-xs font-bold tracking-wider border transition-colors"
                style={{ borderColor: "#333", color: "#9CA3AF", background: "#1A1A1A" }}>
                <Plus size={14} />NEW CAMPAIGN
              </button>
              <button data-testid="ai-insights-btn"
                className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-xs font-bold tracking-wider transition-colors"
                style={{ background: "#1E1E3F", color: "#818CF8", border: "1px solid #312E81" }}>
          ...
          <div className="grid grid-cols-5 gap-4">
            {[
              ...
              { label: "CES SCORE",     value: "4.3", sub: "→ Stable",               bench: "Customer Effort (1-5)",color: "#9CA3AF", Icon: Minus },
            ].map((k, i) => (
              <div key={k.label} data-testid={`kpi-card-${i}`}
                ...
                <div className="flex items-start justify-between mb-3">
                  <span className="text-[10px] font-bold tracking-[0.15em] text-[#4B5563]">{k.label}</span>
                  <k.Icon size={16} style={{ color: k.color }} />
                </div>
                <div className="text-4xl font-bold mb-1 font-['Outfit']" style={{ color: k.color }}>{k.value}</div>
                <div className="text-[11px] mb-0.5" style={{ color: k.color }}>{k.sub}</div>
                <div className="text-[10px] text-[#374151]">{k.bench}</div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-12 gap-4">
            {/* Trend Chart */}
            <div className="col-span-6 rounded-xl p-5 border" style={{ background: "#141414", border: "1px solid #222" }}>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <div className="text-[11px] font-bold tracking-widest text-[#4B5563] uppercase mb-0.5">Trend</div>
                  <h3 className="text-sm font-bold text-white">NPS & CSAT Trend — Last 12 Months</h3>
                </div>
                <button className="text-[10px] font-bold tracking-wider px-3 py-1.5 rounded-lg border"
                  style={{ borderColor: "#333", color: "#4B5563", background: "#1A1A1A" }}>
                  QUARTERLY VIEW
                </button>
              </div>
            ...
            {/* NPS Gauge */}
            <div className="col-span-3 rounded-xl p-5 border flex flex-col" style={{ background: "#141414", border: "1px solid #222" }}>
              <div className="mb-3">
                <div className="text-[10px] font-bold tracking-widest text-[#4B5563] uppercase mb-0.5">Gauge</div>
                <h3 className="text-sm font-bold text-white">Net Promoter Score</h3>
                <span className="text-[10px] text-[#4B5563]">Q2 2026</span>
              </div>
            ...
            {/* CSAT Distribution */}
            <div className="col-span-3 rounded-xl p-5 border" style={{ background: "#141414", border: "1px solid #222" }}>
              <div className="mb-4">
                <div className="text-[10px] font-bold tracking-widest text-[#4B5563] uppercase mb-0.5">Distribution</div>
                <h3 className="text-sm font-bold text-white">CSAT Distribution</h3>
                <span className="text-[10px] text-[#4B5563]">142 RESPONSES</span>
              </div>
              <div className="space-y-3">
                {csatData.map((d, i) => (
                  <div key={d.rating} className="flex items-center gap-3">
                    <span className="text-xs w-7 text-right flex-shrink-0" style={{ color: csatBarColor(i) }}>{d.rating}</span>
                    ...
                    <span className="text-xs text-[#4B5563] w-6 flex-shrink-0">{d.count}</span>
                  </div>
                ))}
              </div>
              <div className="mt-4 pt-3 border-t flex items-center justify-between" style={{ borderColor: "#222" }}>
                <span className="text-[11px] text-[#4B5563]">Avg: <strong className="text-white">4.4 / 5.0</strong></span>
                <span className="text-[11px] font-bold text-[#22C55E]">CSAT: 87%</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {/* Top Pain Points */}
            <div className="rounded-xl p-5 border" style={{ background: "#141414", border: "1px solid #222" }}>
              <div className="flex items-start justify-between mb-4">
                <div>
                  <div className="text-[10px] font-bold tracking-widest text-[#4B5563] uppercase mb-0.5">Analysis</div>
                  <h3 className="text-sm font-bold text-white">Top Pain Points</h3>
                </div>
                <span className="text-[10px] font-bold px-2 py-1 rounded-lg" style={{ background: "#3D1515", color: "#EF4444" }}>
                  NEEDS ACTION
                </span>
              </div>
              <div className="space-y-4">
                {painPoints.map((p) => (
                  <div key={p.label}>
                    ...
                    <div className="text-[10px] text-[#374151] mb-1.5">{p.sub}</div>
            ...
            <div className="rounded-xl p-5 border" style={{ background: "#141414", border: "1px solid #222" }}>
              <div className="flex items-start justify-between mb-4">
                <div>
                  <div className="text-[10px] font-bold tracking-widest text-[#4B5563] uppercase mb-0.5">Strengths</div>
                  <h3 className="text-sm font-bold text-white">What We're Doing Well</h3>
                </div>
                <span className="text-[10px] font-bold px-2 py-1 rounded-lg" style={{ background: "#0F2E1A", color: "#22C55E" }}>
                  SUSTAIN & SCALE
                </span>
              </div>
              <div className="space-y-4">
                {doingWell.map((d, i) => (
                  <div key={i} className="flex gap-3">
                    ...
                    <div>
                      <p className="text-xs text-[#9CA3AF] italic leading-relaxed mb-1">{d.quote}</p>
                      <p className="text-[10px]" style={{ color: "#374151" }}>{d.tag}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-xl p-5 border" style={{ background: "#141414", border: "1px solid #222" }}>
              <div className="flex items-start justify-between mb-4">
                <div>
                  <div className="text-[10px] font-bold tracking-widest text-[#4B5563] uppercase mb-0.5">Accounts</div>
                  <h3 className="text-sm font-bold text-white">Account Health</h3>
                </div>
                <span className="text-[10px] font-bold px-2 py-1 rounded-lg" style={{ background: "#3D2800", color: "#F59E0B" }}>
                  NEEDS REVIEW
                </span>
              </div>
              <div className="space-y-3">
                {accounts.map(a => {
                  ...
                  return (
                    <div key={a.name} className="flex items-center gap-3 p-2 rounded-lg hover:bg-[#1A1A1A] transition-colors">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-[11px] font-bold text-white"
                        style={{ background: "#2A2A2A" }}>{a.initials}</div>
                      <span className="flex-1 text-sm text-[#9CA3AF]">{a.name}</span>
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[11px] font-bold tracking-[0.15em] text-[#4B5563] uppercase">
                RECENT CUSTOMER VERBATIMS — AI CATEGORISED
              </span>
            </div>
            <div className="grid grid-cols-3 gap-4 pb-8">
              {verbatims.map((v, i) => (
                <div key={i} data-testid={`verbatim-${i}`} className="rounded-xl p-5 border"
                  style={{ background: "#141414", border: "1px solid #222" }}>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-[10px] font-bold text-[#4B5563]">NPS {v.score}</span>
                  </div>
                  <p className="text-sm text-[#9CA3AF] italic leading-relaxed">{v.text}</p>
                </div>
              ))}
            </div>
          </div>
